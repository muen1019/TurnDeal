...
const schema=await read('contracts/result-api.v0.2.schema.json');
const openapi=await read('backend/openapi.json');
const fixture=await read('contracts/fixtures/result-v0.2.json');
const examples=await read('contracts/fixtures/result-api-v0.2.json');
...
assert.deepEqual(Object.keys(openapi.paths['/api/requests/{request_id}/decisions'].post.responses).sort(),['200','400','404','409','410','500','503']);
function refs(value){if(!value||typeof value!=='object')return;for(const [key,v] of Object.entries(value)){
 if(key==='$ref'){
  if(v.startsWith('../contracts/')){assert.ok(v.startsWith('../contracts/result-api.v0.2.schema.json#/$defs/'));assert.ok(schema.$defs[v.split('/').at(-1)]);}
...
console.log('Result v0.2: schema, lifecycle, examples and all OpenAPI references passed.');
