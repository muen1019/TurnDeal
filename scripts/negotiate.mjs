...
for (const arg of args) if (!['--offline', '--memory', '--live'].includes(arg)) throw new Error(`Unknown argument: ${arg}`);
...
const databasePath = args.has('--memory') ? ':memory:' : fileURLToPath(new URL('data/negotiation.sqlite', root));
