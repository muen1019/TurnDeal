...
function verifyDatabase(db) {
  ...
  `).get().count);
  assert.equal(tableCount, 49, "database must include discovery, negotiation, evaluation, Improver, ACP purchase and catalog policy tables");
  for (const name of ['improver_jobs', 'improver_intent_revisions', 'improver_global_preferences', 'improver_workflows']) {
