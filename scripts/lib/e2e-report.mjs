...
const preferenceLabel = report => (report.preferences ?? report.evaluation?.snapshot?.request?.normalized_intent?.preferences ?? []).includes('after_sales_first') ? '售後優先' : '價格優先';
const money = value => value == null ? '—' : `NT$${value}`;
const roundText = round => round.offers.length ? round.offers.map(o => `${o.variant === 'bundle' ? '組合' : '單買'} ${money(o.total_price_twd)}${o.eligibility?.status === 'needs_confirmation' ? '（需加價授權）' : o.eligibility?.status === 'rejected' ? '（未通過驗證）' : ''}`).join(' / ') : round.outcome;
...
export function reportMarkdown(report) {
  ...
  return `# ${report.evaluation ? 'Orchestrator → 協商 → Evaluator 完整測試' : '五種銷售偏好的協商 E2E 結果'}\n\n` +
    ...
    `買家偏好：${preferenceLabel(report)}。\n\n**${report.passed ? 'PASS' : 'FAIL'}**：${report.checks.filter(c => c.passed).length}/${report.checks.length} 項檢查通過。${report.solutions.length} 家 Seller，${report.offer_count} 個最終 Offer，結果狀態 ${report.status}。\n\n` +
    `範圍：${report.pipeline ? `${report.pipeline?.stages?.[0]?.startsWith('Text') ? '文字需求 → 規則 Formatter → SQLite → ' : 'SQLite 已解析需求 → '}既有 handoff.prepare／Discovery → ` : ''}五家 Seller → Buyer／Seller 模型協商 → Backend 驗證 → SQLite 持久化與重開重播。${report.evaluation ? '另包含獨立 Evaluator 排序。' : '尚不包含 Evaluator 排序。'}不包含 HTTP／UI 操作或交易兌換。\n\n` +
    `## 五套 solution\n\n| 賣家／偏好 | 單買 | 組合 | 配送 | 已執行輪次 | 停止原因 |\n| --- | ---: | ---: | --- | ---: | --- |\n${rows.join('\n')}\n\n` +
    `展示設定：A 清庫、B 物流、C 組合／贈品折現、D 回購權益、E 售後服務。未來券不扣本次價格；權益均為已登錄的模擬履約。C 加價組合須另行授權；${report.evaluation ? '本報告依 Evaluator 排名呈現五個賣家方案，完整 Offer 排名如下。' : '這是按賣家分組的展示，不是 Evaluator 推薦排名。'}金額都是模擬商家含稅運 TWD。\n\n` +
    evaluationMarkdown(report) + `## 每輪協商\n\n以下對話由實際結構化決策產生，非模型逐字稿；正式條件以 Backend 驗證的 Offer 為準。\n\n` + report.solutions.map(s => `### ${s.seller_id} — ${s.label}\n\n${s.description}\n\n| 輪次 | Buyer 提出的條件 | Seller 正式回應 | 執行來源 Buyer／Seller |\n| --- | --- | --- | --- |\n` +
      s.rounds.map(r => `| ${r.round} | ${md(r.buyer_message ?? money(r.target_total_twd))} | ${md(r.seller_message ?? roundText(r))} ${md(roundText(r))} | ${r.buyer_provider ?? '—'} / ${r.seller_provider ?? '—'} |`).join('\n') +
...
export function reportHtml(report) {
  ...
  const cards = report.solutions.map(s => {
    const featured = s.recommended ?? (s.bundle?.eligibility?.status === 'eligible' ? s.bundle : s.standalone);
    return `<article><div class="eyebrow">${e(s.seller_id)}${s.sponsored ? ' <span class="sponsor">Sponsored</span>' : ''}</div><h2>${s.ranking ? `#${s.ranking.rank} ` : ''}${e(s.label)}</h2><p class="desc">${e(s.description)}</p>
      ...
      ${s.ranking ? `<p>${e(s.ranking.reason)}</p><ul>${s.ranking.tradeoffs.map(t => `<li>${e(t)}</li>`).join('')}</ul>` : ''}<dl><dt>單買</dt><dd>${e(money(s.standalone?.total_price_twd))}</dd><dt>搭售選項</dt><dd>${e(money(s.bundle?.total_price_twd))}${s.bundle?.eligibility?.status === 'needs_confirmation' ? '（需加價授權）' : ''}</dd><dt>停止原因</dt><dd>${e(s.stop_reason)}</dd></dl>
      <details open><summary>查看 ${s.rounds.length} 輪協商</summary>${s.rounds.map(r => `<div class="round"><b>Round ${r.round}</b><p><b>Buyer：</b>${e(r.buyer_message ?? money(r.target_total_twd))}</p><p><b>Seller：</b>${e(r.seller_message ?? roundText(r))}</p><p class="note">${e(roundText(r))}</p><small>${e(r.buyer_provider)} / ${e(r.seller_provider)}</small></div>`).join('')}</details>
      ${(s.standalone?.benefits ?? []).length ? `<details open><summary>已登錄權益與使用條件（模擬）</summary>${s.standalone.benefits.map(b => `<p><b>${e(b.description)}</b><br>${e(b.conditions)}</p>`).join('')}</details>` : ''}
      <details><summary>Offer ID 與授權狀態</summary>${[s.standalone, s.bundle].filter(Boolean).map(o => `<p>${e(o.variant)}<br><code>${e(o.offer_id)}</code></p>`).join('')}</details></article>`;
  }).join('');
...
function evaluationHtml(report) {
  ...
  return `<section><h2>完整 Offer 排名</h2><p>Evaluator：${e(evaluation.provider)} · ${e(evaluation.model ?? 'offline')} · ${report.evaluation_duration_seconds} 秒 · ${evaluation.usage.calls} 次 API · ${evaluation.usage.actual_tokens} tokens</p><p>fallback：${e(evaluation.fallback_reason ?? '無')}。所有合格 Offer 都被排名；採用與購買需由使用者另外確認。</p><ol>` +
    evaluation.snapshot.ranked_offers.map(r => {
      const o = evaluation.snapshot.offers.find(o => o.offer_id === r.offer_id);
