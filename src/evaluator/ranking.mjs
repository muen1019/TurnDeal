...
export function deterministicRanking(input) {
  ...
  const priority = input.intent.ranking_weights&&!input.intent.preferences.length?'偏好權重綜合':{ price_first: '價格', delivery_first: '配送', trust_first: '信任', after_sales_first:'已登錄售後條件' }[input.intent.preferences[0] ?? 'price_first'];
