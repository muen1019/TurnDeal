...
4. 後端建立一個問題子請求（202），使用 GPT-4.1 根據原 intent、偏好及拒絕原因提出 1–3 個具體問題／快捷選項。未設定 key、超時或不合法輸出使用明確標示的規則追問。
...
原有硬預算和交期保留；只有買家明確的新回答可修改，不能把參考選項当授權。重新篩選不保證出現全新商品：目前只有已設定策略的 A–E 賣家可議價，條件未變時可能重複；不會為湊數放寬硬限制。
...
- Formatter 與拒絕追問預設 GPT-4.1，可用後端 `OPENAI_FORMATTER_MODEL` 覆寫。其他議價／Evaluator 模型不變；key 仍只透過安全終端輸入。
