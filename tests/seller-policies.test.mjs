...
function ready() {
  ...
  const s = p.sellers[0];
  s.status = 'ready';
  s.settings = { strategy: 'balanced', max_rounds: 2, timeout_ms: 3000, quote_ttl_seconds: 300, buyer_below_floor: 'counter_at_floor', after_last_round: 'stop' };
  s.listings = [{ listing_id: 'listing_002', terms: {
    floor_item_price_twd: 649, round_discount_twd: [20,60],
...
test('invalid and incomplete policies are rejected', () => {
  const changes = [
    ...
    p => p.sellers[0].settings.timeout_ms = 3001,
    ...
    p => p.sellers[0].listings[0].terms.round_discount_twd = [60,20],
    p => p.sellers[0].listings[0].terms.round_discount_twd = [20,61],
