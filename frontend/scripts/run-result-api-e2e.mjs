...
try {
 ...
 web=await preview({root,preview:{port:5188,strictPort:true,host:'127.0.0.1',proxy:{'/api':`http://127.0.0.1:${api.address().port}`}}});
 ...
 if(web)await new Promise(done=>web.httpServer.close(done));if(api)await new Promise(done=>api.close(done));app.locals.store.close();
