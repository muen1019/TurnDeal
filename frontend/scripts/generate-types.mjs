...
const source=JSON.parse(await readFile(new URL('../../contracts/result-api.v0.2.schema.json',import.meta.url),'utf8'));
// Flatten validation conditionals for type generation only. Runtime Ajv keeps every constraint.
function simplify(value){if(Array.isArray(value))return value.map(simplify);if(!value||typeof value!=='object')return value;return Object.fromEntries(Object.entries(value).filter(([k])=>!['allOf','if','then','else','contains','minContains','maxContains'].includes(k)).map(([k,v])=>[k,simplify(v)]));}
...
const text=await compile(root,'CommerceTypes',{bannerComment:'/* Generated from contracts/result-api.v0.2.schema.json. Run npm run generate:types; do not edit. */',style:{singleQuote:true},additionalProperties:false});
