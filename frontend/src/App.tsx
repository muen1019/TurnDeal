...
import {OfferDeck,OfferDetails,FeedbackForm} from './components/offers';
...
import {DecisionSummary} from './components/offers/DecisionSummary';
...
export default function App(){
 ...
 if(progressStatus&&import.meta.env.DEV&&import.meta.env.VITE_OFFERMESH_MOCK)messages.push({id:'mock-progress-'+c.id,role:'system',content:'模擬進度 · 固定示範無線滑鼠、預算 NT$1,000、7 天內送達，不會購買或付款。'});
 ...
 if(s.view==='chat'&&!['accepted','rejected'].includes(snapshot?.status??''))content=<><ChatPanel messages={messages} status={chatStatus} progressStatus={progressStatus} progressError={agentProgress.error} teaser={ready?{requestId:snapshot.request_id,title:'查看優惠組合',offerCount:snapshot.ranked_offers.length,lowestTotalTwd:Math.min(...snapshot.ranked_offers.map(r=>snapshot.offers.find(o=>o.offer_id===r.offer_id)!.total_price_twd)),imageUrl:undefined}:undefined} draft={c.draft} onDraft={draft=>patch({draft})} onSend={s.send} onOpenOffers={id=>go('offers',id)} sending={pending||s.saving||processing(snapshot?.status)} error={s.error} unsavedDefinitions={d.intent!==d.savedIntent||d.preference!==d.savedPreference} onEditDefinitions={()=>go('settings')}/>{definitionEditor}{(pending||snapshot&&snapshot.status!=='awaiting_user'&&!processing(snapshot.status))&&<div className="chat-context-actions">{s.unknown&&<button className="button secondary" disabled={s.busy} onClick={s.retry}>核對原提交</button>}{snapshot&&<button className="text-button" onClick={()=>go('offers')}>查看本輪狀態 <ArrowRight size={16}/></button>}</div>}</>;
 else if(s.view==='settings')content=<div className="workspace-content editor-view content-enter">{back}{definitionEditor}</div>;
   ...
   const detail=snapshot.offers.find(o=>o.offer_id===s.detailId);
   content=<div className="workspace-content">{notice}{detail?<OfferDetails snapshot={snapshot} offer={detail} ranking={snapshot.ranked_offers.find(r=>r.offer_id===detail.offer_id)??null} pending={pending||!verified} expired={expired(detail)} onBack={()=>go('offers')} onAccept={id=>s.accept(id)}/>:<Status title="找不到這個方案" text="此明細不屬於本輪結果。" action={<button className="button secondary" onClick={()=>go('offers')}>返回優惠</button>}/>}</div>;
   ...
   content=<div className="workspace-content content-enter">{back}{notice}{selected?<><DecisionSummary snapshot={snapshot} offer={selected}/>{import.meta.env.VITE_OFFERMESH_MOCK?<p className="commerce-panel">互動預覽不連接结帳。請在本機 5173 的完整服務測試購買。</p>:<PurchasePanel key={snapshot.request_id} requestId={snapshot.request_id} offerId={selected.offer_id} profile={buyer.profile}/>}{!import.meta.env.VITE_OFFERMESH_MOCK&&<ImprovementPanel key={snapshot.request_id+"improve"} requestId={snapshot.request_id} onNext={s.openFollowup} onNew={s.newConversation} accepted/>}</>:<Status title="方案資料不完整" text="無法確認被採用的方案，請重新載入。"/>}<button className="text-button new-round-link" onClick={s.newConversation}>開始新的購物需求</button></div>;
 }else if(snapshot.status==='rejected'&&snapshot.decision?.action==='reject')content=<div className="workspace-content content-enter">{back}{notice}{import.meta.env.VITE_OFFERMESH_MOCK?<Status title="回饋已保存" text="互動預覽不執行 Improver，請在本機 5173 測試完整流程。" action={<button className="button secondary" onClick={s.newConversation}>新需求</button>}/>:<ImprovementPanel key={snapshot.request_id} requestId={snapshot.request_id} onNext={s.openFollowup} onNew={s.newConversation}/>}</div>;
