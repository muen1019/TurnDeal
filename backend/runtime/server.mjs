...
const app=createRuntimeApp({dbPath:process.env.OFFERMESH_DB_PATH??resolve(root,'data/app.sqlite'),apiKey,improverOptions:{mode},purchaseOptions:{mode:process.env.OFFERMESH_PURCHASE_MODE??'test'},negotiationDelayMs:2800,evaluationDelayMs:1000});
const server=app.listen(Number(process.env.PORT??3201),'127.0.0.1',()=>{
  console.log(`OfferMesh integrated API: http://127.0.0.1:${server.address().port} (${mode}; 20 configured sellers; ACP test purchase, simulated payment)`);
});
