...
import {effectivePreferenceText} from '../dist/src/improver/preferences.js';

/** A new round reads the latest user profile atomically; existing children remain immutable. */
export function ensureImprovementChild(store,job){
  return store.transaction(()=>{
    ...
    const preference=store.preferences.current(w.buyer_id);
    if(preference.issues?.length)throw new Error('child_formatter_unsupported');
    const documents={...job.result.documents,preference_md:preference.markdown};
    if(documents.revision!==parent.documents.revision+1)throw new Error('revision_mismatch');
    const result=formatIntent({intent_md:documents.intent_md,preference_md:effectivePreferenceText(preference)},preference.saved_preferences??[]);
    if(preference.ranking_weights&&result.normalized_intent)result.normalized_intent.ranking_weights=preference.ranking_weights;
    if(result.status!=='ready'||!result.normalized_intent)throw new Error('child_formatter_unsupported');
    const child=store.create(w.buyer_id,documents,undefined,undefined,undefined,{parent_request_id:parent.request_id,root_request_id:parent.root_request_id,revision:documents.revision,preference}).body;
