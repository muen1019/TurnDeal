...
describe('unified v0.3 contract and persistence',()=>{
 ...
 it.each(['accept','reject'] as const)('migrates legacy %s with backup, original quotes and exact replay',async action=>{
  ...
  expect(persisted.exec('PRAGMA foreign_key_check')).toEqual([]);
  ...
  expect(persisted.exec('SELECT version FROM schema_migrations ORDER BY version')[0].values.map(row=>row[0])).toEqual(versions);
